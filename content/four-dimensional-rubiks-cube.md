...
Summary: Porting a dormant ~11-year-old Java applet of a 4D Rubik's cube to a modern web app with Claude Code.

I recently used [Claude Code](https://claude.com/claude-code) to revive a project that had been sitting dormant for about 11 years: a four-dimensional Rubik's cube. The original was a Java applet — a technology that modern browsers no longer run — and porting it to a modern web stack brought it back to life.
...
![A four-dimensional Rubik's cube]({static}/images/magiccube4d.jpg)
...
Everything after that — the other 74 commits — was tweaking: interface experiments, a puzzle gallery,
a three-dimensional cube running on the same engine, and a great deal of deciding what *not* to add.
Reviving the dead applet was a day's work. Working out what it should feel like took the rest of the
week.
...
spoiling the thing I most wanted to keep. The original MagicCube4D is one window with a trackball:
...
viewport, the original colours on the original sky-blue background, and a trackball you drag. There
...
showing where the four axes currently point, with each spoke coloured like the cell that sits on it.
The axis pointing straight at you is not drawn at all — that is the cell the renderer hides so you
can see into the interior, and labelling a cell you cannot see would be a lie. At the `+X` viewpoint,
the label that vanishes is `−X`.
...
The thing that kept surprising me as I worked on this is that **nothing in the engine is about the
hypercube.** A puzzle is a Schläfli product symbol and an edge length, and that is the whole
...
All of them are playable with exactly the same rules: click a sticker, the number of colours it
...
The part that surprised me most is that an ordinary Rubik's cube came almost for free.
...
and turning off one thing. The renderer hides the cell nearest you so that you can see through it
...
from how many colours the piece you clicked carries: a corner asks for a rotation about a vertex, an
...
perfectly real — but it turns the *whole cube* by 120° rather than a layer. The reason is that in
...
### And what it cost the atmosphere

Having counted the tokens, I wanted to know what they cost in carbon. This turns out to be
surprisingly hard to answer honestly, and the way it is usually answered is wrong.

If you ask a search engine, you get something like "0.45 to 20 grams of CO₂e per 1,000 output
tokens", which for 2.8 million output tokens gives anywhere from 1.3 kg to 57 kg — the high end
being about a 140-mile drive. Two things are wrong with that. The range is a **44× spread**, which is
not an estimate so much as an admission that nobody knows, and quoting it to three significant figures
lends it a precision it does not have. More importantly, it prices only the *output* tokens, which is
...
| | tokens | gCO₂e | share |
|---|---:|---:|---:|
| Cache reads | 1,066,103,626 | 3,326 | 50% |
| Output | 2,848,899 | 2,353 | 36% |
| Cache creation | 24,477,308 | 955 | 14% |
| Fresh input | 9,081 | 0.4 | — |
| **Total** | | **≈ 6,634 g** | |
...
tonnes. For context, it is a rounding error next to the flight I would have taken to a conference to
talk about it — but it is not nothing, and I would rather publish the uncertainty than a number that
looks authoritative because it has three digits in it.

## Reflections

<!-- TODO: Closing thoughts on reviving old code, the math, community stewardship, and agentic AI. -->
